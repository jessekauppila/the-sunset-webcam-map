...
export async function createScene(input: SceneCreateInput): Promise<number> {
  const rows = await sql`
    INSERT INTO kiosk_scenes
      (label, tags, notes, represents_at, window_minutes, source, state, provenance)
    VALUES (${input.label}, ${input.tags}, ${input.notes}, ${input.representsAt},
            ${input.windowMinutes}, ${input.source},
...
export async function listScenes(): Promise<SceneSummary[]> {
  const rows = await sql`
    SELECT id, label, tags, represents_at, window_minutes, source, created_at
...
export async function getScene(id: number): Promise<Scene | null> {
  const rows = await sql`
    SELECT id, label, tags, notes, represents_at, window_minutes,
           source, state, provenance, created_at
