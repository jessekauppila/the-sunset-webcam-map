...
export function RulesBox({ dials: d, version }: { dials: SoloDials; version?: SoloVersionSpec }) {
  ...
  return (
    <div style={box} title="The ordering rules, in the order they apply, with the current dial values substituted.">
      ...
      <div><B>3.</B> In a bin{d2?.cameraRun ? <>, <B>cameras</B> (a camera&apos;s frames are one item; a dwell plays them oldest to newest)</> : null}: never shown first, then longest since shown, then best score{d.promoteNew ? ', new frames +0.10' : ''}{rhythm ? <>;{rhythm}</> : null}.</div>
