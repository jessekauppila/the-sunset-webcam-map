 ...
 * and its queue as the STUDIO dials would order them. With solo2's camera
 * run on, a camera is one box wherever it sits (camera-run spec §5.2):
 * its frames stacked oldest to newest, the newest last with the row's
 * annotations, and every frame of a queued or on-glass camera leaves the
 * bins. The screen itself is drawn above, by GlassPreview, so nothing here
 * repeats the picture.
 */
