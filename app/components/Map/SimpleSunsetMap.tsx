...
// Add debugging for the token
console.log('🗝️ Mapbox token present:', !!mapboxgl.accessToken);
...
export default function SimpleSunsetMap({
  ...
  if (isLoading) {
    return (
      <div
        className={`h-96 bg-gray-100 flex items-center justify-center mb-8`}
      >
  ...
  if (error) {
    return (
      <div
        className={`h-96 bg-red-50 flex items-center justify-center mb-8`}
      >
  ...
  return (
    <div
      className={`h-96 bg-white border border-gray-300 rounded overflow-hidden mb-8`}
    >
