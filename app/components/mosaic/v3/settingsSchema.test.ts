...
describe('V3_SETTINGS_SCHEMA', () => {
  it('has a knob for every composition decision', () => {
    const keys = V3_SETTINGS_SCHEMA.map((k) => k.key);
    for (const key of [
  ...
  it('groups knobs into rail sections', () => {
    const sections = new Set(V3_SETTINGS_SCHEMA.map((k) => k.section));
  ...
  it('defaults gateThreshold to 0.55 on the probability scale', () => {
    const gate = V3_SETTINGS_SCHEMA.find((k) => k.key === 'gateThreshold')!;
