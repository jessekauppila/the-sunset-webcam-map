...
- Live check 2026-09-04 (dev server, `.env.local` pool, real time near-simultaneous reloads): v4 counts line `sunset · tiles 15 · evicted 21 · dropped 0 · skipped 0 · held 0 · scale 1.00 · bands 8 full`; v3 `sunset · tiles 11 · evicted 25 · dropped 0 · skipped 0 · scale 1.00 · bands 8 full`; v4 with dials off (`&changeSpreadMs=0&transitionStyle=dissolve&exitTaperDeg=0&missGraceCycles=0`) `sunset · tiles 15 · evicted 21 · dropped 0 · skipped 0 · held 0 · scale 1.00 · bands 8 full`.
  ...
  - The "dials off should byte-match v3" check did **not** converge on this live pool: v4-dials-off held at 15/21 across two reloads (matching v4-dials-on exactly), while v3 read 11/25 (and drifted again to 24→25 evicted on its own re-reload seconds apart, tiles staying at 11). Since v4-dials-off was stable across reloads and differs from v3 by a constant split (not the kind of one-or-two-camera jitter the v3-vs-v3 reload showed), this reads as a real composition difference on live data, not just pool-timing noise — but the frozen-fixture `realPool.test.ts` fixture (Task 2) does confirm exact tile-ID parity at `exitTaperDeg: 0`. Root cause not chased further per this task's scope (verification/recording only, no source edits); flagging for the controller as a live-check anomaly worth a follow-up look, most likely a difference in how v3 vs v4's kiosk tick/session state is seeded or persisted server-side rather than in the shared composition math itself.
