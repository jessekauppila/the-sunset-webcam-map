...
- realPool re-pin: 8 drawn / 34 evicted / 3 passers (sunset), 9 drawn (sunrise) → unchanged at the same four counts, because on the frozen fixture the exit taper touches one passer's height (id `28629957`, altitude −19.3°, 4.7° inside the −24° night edge: 204.3px → 191.8px) without flipping any tile-vs-tile collision, so admitted/evicted tile-ID sets stay byte-for-byte identical between `exitTaperDeg: 0` and the default `6`.
- Suite: `241 passed (241)` test files, `1954 passed (1954)` tests. Build: `✓ Compiled successfully in 9.9s`, full production build finished clean (no errors outside `v4/`; nothing under `v1/`, `v2/`, `v3/` touched — confirmed by `git status`/build log).
- Live check 2026-09-04 (dev server, `.env.local` pool, real time near-simultaneous reloads): v4 counts line `sunset · tiles 15 · evicted 21 · dropped 0 · skipped 0 · held 0 · scale 1.00 · bands 8 full`; v3 `sunset · tiles 11 · evicted 25 · dropped 0 · skipped 0 · scale 1.00 · bands 8 full`; v4 with dials off (`&changeSpreadMs=0&transitionStyle=dissolve&exitTaperDeg=0&missGraceCycles=0`) `sunset · tiles 15 · evicted 21 · dropped 0 · skipped 0 · held 0 · scale 1.00 · bands 8 full`.
  - `[data-testid=v4-setup-counts]` exists and contains `held` — confirmed.
  - v4 tile count (15) ≥ v3 tile count (11) on the same wall — confirmed (pool totals summed to 36 tiles+evicted on both versions across all three near-simultaneous loads, so this was the same underlying pool).
  - The "dials off should byte-match v3" check was **inconclusive**, not a failure: the two tabs composed different pool snapshots. Each tab runs its own dwell/hysteresis history from its own load moment, so v4-dials-off (15/21) and v3 (11/25) were never looking at the same admitted set — v3 alone drifted 24→25 evicted between two of its own reloads seconds apart. The parity claim is instead made where it can be made: the controller ran v3 and v4 (with `exitTaperDeg: 0`) over the frozen live-capture fixture through `compose` and got **byte-identical layouts on both panels** — sunset 8 tiles, sunrise 9, same tile IDs in the same order. That is the fork-is-faithful evidence; the live tab comparison is not.
- Final-review fix wave (commit `e531be8c5`): **F1** `commit()`'s mid-entry branch matched on `fadeEnd` alone and so also caught an entry whose `startAt` was still in the future — the one `clearedAt` had pushed out behind a departure — restarting it at `now` and drawing two cameras in one rect; the branch is now split (in-progress redirects go through `clearedAt`, not-yet-begun entries keep their schedule and re-check clearance) and both record the real delay. **F2** the render loop sleeps between scheduled changes, so the wake frame carried seconds of `dt` and drift's `k = 1 - 0.001^(dt/tau)` went to ~1 (95 % of a 100 px move in one frame); the chase step is now capped at 100 ms in `sample()` and the canvas caps its own `dt` to match. **F3** the canvas promoted a pending frame for a tile that had already left the pool, swapping the picture mid-exit against spec §6; promotion is now guarded on the tile still being in `byId`, and the park branch ignores a pending stamp already in the past.
- Spec corrections: §6.3 says exits tween "regardless of motionMode"; the implementation and its tests instead treat `cut` as a snap (a cut that fades is not a cut) — read §6.3 as tween|drift, not "regardless of motionMode." §7.3: the delay map now includes departing ids and records the start *after* the fade-through wait, not before.
- Other findings from earlier task reviews worth keeping on record: v3's exit fade was never actually drawn — the canvas forgot a departed tile's image immediately, so the fade transition was invisible on the glass; this is fixed in v4's canvas.
